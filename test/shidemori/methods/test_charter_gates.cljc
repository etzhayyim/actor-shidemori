...
(defn- lex [name] (edn/read-string (slurp (java.io.File. lexdir name))))
...
(deftest test-g3-no-afterlife-doctrine-imposed
  (is (= (get (consts (lex "memorialNftAttestation.edn")) "afterlifeDoctrineImposed") false)))

;; ── G7 — no embalming chemicals ──
(deftest test-g7-no-embalming
  (is (= (get (consts (lex "externalMortuaryEngagement.edn")) "embalmingChemicalsUsed") false)))

;; ── G10 — waqf-inalienable cemetery + biodegradable green burial ──
(deftest test-g10-waqf-green-burial
  (let [c (consts (lex "cemeteryLandAttestation.edn"))]
...
(deftest test-silen-review-dignity-ledger
  (let [c (consts (lex "silenShidemoriReview.edn"))
...
(deftest test-g9-free-conscience
  (let [doc (lex "memorialNftAttestation.edn")]
...
(deftest test-cross-doctrinal-open
  (is (= (get (consts (lex "chinkonRemembranceAttestation.edn")) "openToCommunityAttested") true)))
